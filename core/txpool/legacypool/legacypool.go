...
func (pool *LegacyPool) Pending(filter txpool.PendingFilter) map[common.Address][]*txpool.LazyTransaction {
	...
	for addr, list := range pool.pending {
			...
			for i, tx := range txs {
				if filter.MinTip != nil {
					if tx.EffectiveGasTipIntCmp(filter.MinTip, filter.BaseFee) < 0 {
						log.Debug("[TPS-PROF] Pending: MinTip filter truncated",
							"addr", addr,
							"atIndex", i,
							"origLen", origLen,
							"txHash", tx.Hash(),
				...
				if filter.GasLimitCap != 0 {
					if tx.Gas() > filter.GasLimitCap {
						log.Debug("[TPS-PROF] Pending: GasLimitCap filter truncated",
		...
		if filter.MaxDATxSize != nil {
			for i, tx := range txs {
				...
				if estimate.Cmp(filter.MaxDATxSize) > 0 {
					log.Debug("[TPS-PROF] Pending: MaxDATxSize filter truncated",
						"addr", addr, "atIndex", i, "origLen", origLen,
		...
		if origLen > 0 && len(txs) == 0 {
			log.Debug("[TPS-PROF] Pending: ALL txs filtered for account",
...
func (pool *LegacyPool) Add(txs []*types.Transaction, sync bool) []error {
	...
	if len(txs) == 1 {
		log.Debug("[RPC-PROF] txpool.Add breakdown",
